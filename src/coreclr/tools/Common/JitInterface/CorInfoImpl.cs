...
namespace Internal.JitInterface
{
    ...
    internal sealed unsafe partial class CorInfoImpl
    {
        ...
        private bool IsAsyncQuirkMethod(MethodDesc method) => method.IsAsyncVariant() && method.IsAsyncThunk();

        private sealed class QuirkedMethodIL : MethodIL
        {
            ...
            public QuirkedMethodIL(MethodDesc variant, MethodIL ecmaIL)
        ...
        private MethodIL GetMethodILWithAsyncQuirk(MethodDesc method)
        {
            if (IsAsyncQuirkMethod(method))
            {
                ...
                MethodIL methodIL = new QuirkedMethodIL(methodDef, _compilation.GetMethodIL(targetMethodDef));
                if (method != methodDef)
                {
        ...
        private MethodIL RemoveAsyncQuirk(MethodIL methodIL)
        {
            if (methodIL.GetMethodILDefinition() is QuirkedMethodIL)
            {
        ...
        private void PublishCode()
        {
            ...
            CodeBasedDependencyAlgorithm.AddDependenciesDueToMethodCodePresence(ref _additionalDependencies, _compilation.NodeFactory, MethodBeingCompiled, RemoveAsyncQuirk(methodIL));
...
#pragma warning restore CA1822 // Mark members as static
        ...
        private bool getMethodInfo(CORINFO_METHOD_STRUCT_* ftn, CORINFO_METHOD_INFO* info, CORINFO_CONTEXT_STRUCT* context)
        {
            ...
            MethodIL methodIL = method.IsUnboxingThunk() ? null : GetMethodILWithAsyncQuirk(method);
        ...
        private CorInfoInline canInline(CORINFO_METHOD_STRUCT_* callerHnd, CORINFO_METHOD_STRUCT_* calleeHnd)
        {
            ...
            if ((rootModule != calleeModule) && (rootModule != null) && (calleeModule != null))
            {
                ...
                {
                    var calleeIL = GetMethodILWithAsyncQuirk(calleeMethod);
...
#pragma warning restore CA1822 // Mark members as static
        ...
        {
            var methodIL = GetMethodILWithAsyncQuirk(HandleToObject(ftn));
        ...
        private bool resolveVirtualMethod(CORINFO_DEVIRTUALIZATION_INFO* info)
        {
                ...
                )
            {
                ...
                MethodILScope scope = jitInterface.GetMethodILWithAsyncQuirk(methodWithToken.Method);
...
#else
                _compilation.NodeFactory.MetadataManager.GetDependenciesDueToAccess(ref _additionalDependencies, _compilation.NodeFactory, RemoveAsyncQuirk((MethodIL)methodIL), method);
#endif
